...
import type { ImgPipelineConfigUpdate, ImgPipelineManager } from "./manager.js";

export const createImgPipelineRoutes = ({
  notifyLocalOnly,
  runImgPipelineQuery,
  normalizeStringList,
  resolveUserPath,
  ...
  normalizeStringList: (value: unknown) => string[] | null;
  ...
  imgPipelineManager?: ImgPipelineManager | null;
  ...
  const statusRoute: RouteHandler = async (req, res, url) => {
    if (url.pathname !== "/api/pipeline/status") return false;
    if (notifyLocalOnly && !isLocalRequest(req)) {
      sendJson(res, 403, { error: "Forbidden" });
      return true;
    }
    if (!imgPipelineManager) {
      sendJson(res, 200, { success: false, error: "img-pipeline manager not configured" });
      return true;
    }

    if (req.method === "GET") {
      sendJson(res, 200, { success: true, ...imgPipelineManager.getStatus() });
      return true;
    }
    ...
    if (req.method === "POST") {
      try {
        ...
        const updates: ImgPipelineConfigUpdate = {};
        ...
        if (typeof enabled === "boolean") {
          const result = imgPipelineManager.setEnabled(enabled);
          if (!result.ok) {
  ...
  const fileRoute: RouteHandler = async (req, res, url) => {
    if (url.pathname !== "/api/pipeline/file") return false;
    if (notifyLocalOnly && !isLocalRequest(req)) {
      sendJson(res, 403, { error: "Forbidden" });
      return true;
    }
    ...
    if (stat.size > imgPipelineMaxBytes) {
